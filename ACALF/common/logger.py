...
class AverageMeter:
    ...
    def loss_plot(self, plot_idx = 10):
        if len(self.loss_buf) % plot_idx == 0:
            loss_buf = torch.stack(self.loss_buf)
            loss_buf = loss_buf.detach().clone().cpu().numpy()
            plt.plot(loss_buf)
...
class Logger:
    ...
    def initialize(cls, args, training):
        ...
        if not training:
            if args.vote:
                ...
                test_method = 'post_average'
        logpath = os.path.join(args.logpath, 'train/fold_' + str(args.fold) + logtime) if training \
                        else os.path.join(args.logpath, 'test',args.test_dataset + '_'+ args.backbone + '_' + test_method + '_' + str(args.nshot)+ 'shot' + logtime)
    ...
    def save_model_miou(cls, model, epoch, val_miou):
        torch.save(model.state_dict(), os.path.join(cls.logpath, str(epoch) + '_best_model.pt'))
